...
if(window.location.href == "http://www.iloveradio.de/iloveradio/?popup"){
	...
	(function() {
		...
		var ilr = {
			...
			operators: ['Hannes'],
			prefix: 'hi ',
			welcomes: ['hey', 'hall', 'jo', 'guten'],
			byes: ['bye', 'tschüss', 'tschau', 'ciao'],
			users: [],
			blacklist: [],
			noPerms: 'Keine Rechte!',
			commands: {
				help: { command: 'alx',
					action: 'var coms = "";'+
					'for (var key in ilr.commands) {'+
						...
						'coms = coms +ilr.commands[key].command+" ";'+
					'}'+
					'sendMessage('+
					'coms);'},
				 bot: { command: 'boooot',
					action: 'sendMessage("gist{PUNKT}g1thub{PUNKT}com/Bluscream/d0f3bf4f4e380e528cb60f51aafc28c7");'},
				playing: { command: 'song',
					action: 'var _radio = $(".channel-headline.big").text();'+
					'var _artist = $(".channelinfo>.artist").text().toTitleCase();'+
					'var _title = $(".channelinfo>.title").text().toTitleCase();'+
					'sendMessage(_radio+" spielt gerade \'"+_title+"\' by \'"+_artist+"\'.");'},
				time: { command: 'time',
					action: 'var _now = new Date();sendMessage(_now.toString());'},
				last: { command: 'last',
					action: 'sendMessage("UPPER: \'"+lastMessage.name.toUpperCase()+"\' lower: \'"+lastMessage.name.toLowerCase()+"\'");'},
				love: { command: 'love',
					action: 'switch(args.length){'+
					'case 1:'+
						'sendMessage(msg.name+" <3 "+args[0]);break;'+
					'case 2:'+
						'sendMessage(args[0]+" <3 "+args[1]);break;'+
				'};'},
				credits: { command: 'credits',
					action: 'sendMessage("Thanks to Angi, FaridBang and Rooz.");'},
				info: { command: 'info',
					action: 'sendMessage("Boot written as ECMS Java Userscrlpt by Bluscream with <3");'},
				calc: { command: 'calc',
					action: 'if(args.length == 3){var result = 0;switch(args[1]){'+
				...
				uinfo: { command: 'uinfo',
					action: 'var _user = args.toString().replaceAll(","," ");'+
					...
					'};'},
				ulist: { command: 'ulist',
					action: 'sendMessage(ilr.users.toString());'},
				ulength: { command: 'ulength',
					action: 'sendMessage("User Database contains "+ilr.users.length.toString()+" users.");'},
				angi: { command: 'angi',
					action: 'sendMessage("<3 Angi isch kuhl oke <3");'},
				jessi: { command: 'jessi',
					action: 'sendMessage("<3 Jessi is love, Jessi is life! <3");'},
				contact: { command: 'nice',
					action: 'sendMessage("Steam/G1thub/Snapchat/KIK: Bluscream | Skype: bluscream[@]outlook[PUNKT]de | Insta: @realbluscream");'},
			},
			admincommands: {
				...
				'sag': 'sendMessage(msg.name+" sagt \\""+args.toString().replaceAll(","," ")+"\\".");',
				'start': 'sendMessage("Tschau");window.location.reload();',
		...
		ilr.welcome = 'logged in. Use '+ilr.prefix+ilr.commands.help.command+' to get a list.';
			...
			ilr3.shoutbox.get = function(msg) {
				...
				if(msg.name != ilr.name){
						...
						if(msg.name.contains('I')){
							...
							for (var i = 0; i < _length; i++) {
								if(_text.contains(ilr.welcomes[i])){
									if(welcome == "true"){sendMessage('Hallo '+msg.name+' <3');}									
								}
							}
					}
					...
					$.each( ilr.byes, function(i,e){
						if(_text.contains(e)){
							if(bye == "true"){sendMessage('Tschüss '+msg.name+' <3');}
						}
					});
				}
					...
					for (var key in ilr.admincommands) {
						...
						if(cmd == command){
							...
							if($.inArray( msg.name, ilr.operators ) != -1){
								...
								if(disabled == "false"){sayMessage(msg.name, 'Keine rechte für '+command+'.');break;}
							}
						}
					}
					if(cmd == "eval"){
						if($.inArray( msg.name, ilr.operators ) != -1){
							...
							if(disabled == "false"){sayMessage(msg.name, 'Keine rechte für '+cmd+'.');}
						}
					}
				}
				lastMessage = msg;
